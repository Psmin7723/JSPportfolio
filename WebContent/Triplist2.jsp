...
<%
			ArrayList<ListTripDTOOut> List = (ArrayList<ListTripDTOOut>)request.getAttribute("LISTTRIP2");
		String type;
		String level;								
...
<body id="top1">
		...
		<header class="header">
			...
			<h1 class="logo"><a href="/portfolio/main.user">자연게시판</a></h1>
			<a href="#" class="btn_search"><i class="fa fa-search headericon2"></i></a>
			
			<div class="bar-search">
				<div class="inner">
					<form name="mysearch" action="listTrip2.board">
		...
		<section class="adminbox">
				...
				<table class="list">
					...
					%>
						<tr>
							...
							<td class="contenttrip"><a href="read2.board?num=<%=num%>&value=1&pageNo=<%=pageNo%>&pageSize=<%=pageSize%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>&search=<%=word%>&type=<%=type%>&level=<%=level%>"><%=title %></a></td>
				...
				<% 
					...
					{					
				%>
					<a href="listTrip2.board?pageNo=<%=prevPage%>&pageSize=<%=pageSize%>&search=<%=word%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>"><i class="fa fa-angle-double-left"></i></a>
				<% 		
					}
				%>
				<%
				...
				%> 
					 <%if(i == pageNo) {%><!-- 현재 페이지 페이지링크 안됨 -->
		 			 <Strong><%=i+1 %></Strong> 
					 <%}else{%>
					 <a href="listTrip2.board?pageNo=<%=i%>&pageSize=<%=pageSize%>&search=<%=word%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>"><%=i+1 %></a> 
					 <%}%>
				 <%}%> 
				 <% if(nextPage != -1){ //다음 페이지 이동이 없음%>
					<a href="listTrip2.board?pageNo=<%=nextPage%>&pageSize=<%=pageSize%>&search=<%=word%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>"><i class="fa fa-angle-double-right"></i></a>	
				<% 	}else{ %>
				...
				%>
				 ...
				 <div class="top">
						 ...
						 <a href="/portfolio/write.jsp?type=nature">글쓰기</a>
