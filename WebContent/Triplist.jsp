...
<body id="top1">
		...
		<header class="header">
			...
			<h1 class="logo"><a href="/portfolio/main.user">힐링게시판</a></h1>
			<a href="#" class="btn_search"><i class="fa fa-search headericon2"></i></a>
			
			<div class="bar-search">
				<div class="inner">
					<form name="mysearch" action="listTrip.board">
		...
		<section class="adminbox">
				...
				<table class="list">
					...
					%>
						<tr>
							...
							<td class="contenttrip"><a href="read.board?num=<%=num%>&value=1&pageNo=<%=pageNo%>&pageSize=<%=pageSize%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>&search=<%=word%>&type=<%=type%>&level=<%=level%>"><%=title %></a></td>
				...
				<% 
					...
					{					
				%>
					<a href="listTrip.board?pageNo=<%=prevPage%>&pageSize=<%=pageSize%>&search=<%=word%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>"><i class="fa fa-angle-double-left"></i></a>
				<% 		
					}
				%>
				<%
				...
				%> 
					 <%if(i == pageNo) {%><!-- 현재 페이지 페이지링크 안됨 -->
		 			 <Strong><%=i+1 %></Strong> 
					 <%}else{%>
					 <a href="listTrip.board?pageNo=<%=i%>&pageSize=<%=pageSize%>&search=<%=word%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>"><%=i+1 %></a> 
					 <%}%>
				 <%}%> 
				 <% if(nextPage != -1){ //다음 페이지 이동이 없음%>
					<a href="listTrip.board?pageNo=<%=nextPage%>&pageSize=<%=pageSize%>&search=<%=word%>&viewStart=<%=viewStart%>&viewEnd=<%=viewEnd%>"><i class="fa fa-angle-double-right"></i></a>	
				<% 	}else{ %>
				...
				%>
				 ...
				 <div class="top">
						 ...
						 <a href="/portfolio/write.jsp?type=healing">글쓰기</a>
